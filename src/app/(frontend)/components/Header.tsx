    ...
    <header className={`fixed top-0 left-0 w-full z-50 ${scrolled ? 'scrolled' : 'bg-transparent'}`}>
      <div className="container mx-auto px-4 py-1 flex justify-between items-center">
        <div className="flex items-center">
          <Link href="/" className="text-black font-bold text-2xl flex items-center">
            {logoImage ? (
              <Image 
                src={logoImage.url} 
              ...
              <>
                <div className="w-10 h-10 bg-red-500 rounded-full mr-3 flex items-center justify-center">
                  <span className="text-black font-bold">{logoIcon}</span>
                </div>
                {logoText}
              </>
            )}
        ...
        <nav className="hidden md:flex space-x-6">
          {navigationItems.map((item) => (
            <Link
              key={item.id}
              href={item.link || '#'}
              className={`text-black font-medium hover:text-red-500 transition ${pathname === (item.link || '') ? 'text-red-500' : ''}`}
            >
              {item.label}
            </Link>
          ))}
